...
class Crypto_Analyse:
    ...
    def run(self):
        ...
        name_crypto = list(
            map(lambda current_span: current_span.contents[1], name_crypto))
        value_crypto = list(map(lambda value: float(
            value.get_text()[:-2]), value_crypto))
    ...
    def tri(self, cours=False, value=False):
        if cours:
            self.final_list = sorted(
                self.final_list, key=lambda cours: cours["cours"], reverse=True)
        elif value:
            self.final_list = sorted(
                self.final_list, key=lambda cours: cours["value"], reverse=True)
        else:
            pass

    def connexion_mongodb(self):
        client = MongoClient(
            "mongodb+srv://ID/Crypto?retryWrites=true&w=majority")
        database = client["Crypto"]
        collection = database.cryptomonnaie

        for i in range(len(self.final_list)):
            database.cryptomonnaie.find_one_and_replace(
                {"name": self.final_list[i]["name"]}, self.final_list[i])
    ...
    def ecriture(self):
        with open("table.csv", "w") as table_csv:
            ...
            for i in range(len(self.final_list)):
                table_csv.write("{},{},{}\n".format(
                    self.final_list[i]["name"], self.final_list[i]["cours"], self.final_list[i]["value"]))

    def save_as_json(self):
        with open("table.json", "w") as table_json:
            table_json.write(dumps(self.final_list, indent=4))
